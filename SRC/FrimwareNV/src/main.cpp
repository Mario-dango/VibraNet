...
void setup() {
    ...
    if (client.connect(clientId.c_str())) {
        ...
        if (config.mode == 1) { // BURST
            ...
            if (mpu1_active) performBurstCapture(mpu1, "s1", node_id, client, config);
            if (mpu2_active) performBurstCapture(mpu2, "s2", node_id, client, config);
        } else { // SNAPSHOT
            Serial.println(">> Iniciando Captura SNAPSHOT...");
            if (mpu1_active) runSnapshotMode(mpu1, "s1", client, node_id);
            if (mpu2_active) runSnapshotMode(mpu2, "s2", client, node_id);
        }
        
        if (mpu1_active) setupMPU_Latch(mpu1, MPU1_ADDR, config);
        if (mpu2_active) setupMPU_Latch(mpu2, MPU2_ADDR, config);
