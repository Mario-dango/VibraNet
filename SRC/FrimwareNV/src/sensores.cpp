...
void aggressiveBusRecover() {
    uart_println(">> [1] Lavado de Bus I2C...");
...
void groundingPins() {
    uart_println(">> [2] Grounding SDA/SCL...");
...
void blindSensorReset(uint8_t addr) {
    ...
    char hexAddr[3]; snprintf(hexAddr, sizeof(hexAddr), "%02X", addr);

    uart_print(">> [3] Reset Ciego -> 0x"); uart_println(hexAddr); 
}
